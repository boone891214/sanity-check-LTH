...
INIT_LR=${7:-"0.2048"}
SAVE_FOLDER=${6:-"./checkpoints/resnet18/LTH/pretraining/ep${EPOCHS}/lr${INIT_LR}/cosine_bs_${GLOBAL_BATCH_SIZE}_${WARMUP}ep_warmup"}
...
#SAVE_FOLDER=${6:-"./checkpoints/resnet18/LTH/pretraining/ep${EPOCHS}/lr${INIT_LR}/${WIDTH}_0.914/cosine_bs_${GLOBAL_BATCH_SIZE}_${WARMUP}ep_warmup"}
#mkdir -p ${SAVE_FOLDER}
#python ./multiproc.py --nproc_per_node 8 ./main.py /data/imagenet --data-backend dali-cpu --raport-file raport.json -j8 -p 100 --seed ${SEED} --lr ${INIT_LR} --optimizer-batch-size ${GLOBAL_BATCH_SIZE} --warmup ${WARMUP} --arch ${ARCH} -c fanin --label-smoothing 0.1 --lr-schedule ${LR_SCHEDULE} --mom 0.875 --wd 3.0517578125e-05 --workspace ./ -b ${LOCAL_BATCH_SIZE} ${AMP} --static-loss-scale 128 --epochs ${EPOCHS} --mixup 0.2 --widths=${WIDTH} --sp-config-file=${CONFIG_FILE} --sp-admm-sparsity-type=${SPARSITY_TYPE} ${PRUNE_ARGS} --checkpoint-dir ${SAVE_FOLDER} --log-filename=${SAVE_FOLDER}/log.txt --resume ${LOAD_CKPT}


# WIDTH=${12:-"26-51-110-182-26"}
# SAVE_FOLDER=${6:-"./checkpoints/resnet18/LTH/pretraining/ep${EPOCHS}/lr${INIT_LR}/${WIDTH}_0.832/cosine_bs_${GLOBAL_BATCH_SIZE}_${WARMUP}ep_warmup"}
# mkdir -p ${SAVE_FOLDER}
# python ./multiproc.py --nproc_per_node 8 ./main.py /data/imagenet --data-backend dali-cpu --raport-file raport.json -j8 -p 100 --seed ${SEED} --lr ${INIT_LR} --optimizer-batch-size ${GLOBAL_BATCH_SIZE} --warmup ${WARMUP} --arch ${ARCH} -c fanin --label-smoothing 0.1 --lr-schedule ${LR_SCHEDULE} --mom 0.875 --wd 3.0517578125e-05 --workspace ./ -b ${LOCAL_BATCH_SIZE} ${AMP} --static-loss-scale 128 --epochs ${EPOCHS} --mixup 0.2 --widths=${WIDTH} --sp-config-file=${CONFIG_FILE} --sp-admm-sparsity-type=${SPARSITY_TYPE} ${PRUNE_ARGS} --checkpoint-dir ${SAVE_FOLDER} --log-filename=${SAVE_FOLDER}/log.txt --resume ${LOAD_CKPT}


# WIDTH=${12:-"32-32-128-256-32"}
# SAVE_FOLDER=${6:-"./checkpoints/resnet18/LTH/pretraining/ep${EPOCHS}/lr${INIT_LR}/${WIDTH}_0.738/cosine_bs_${GLOBAL_BATCH_SIZE}_${WARMUP}ep_warmup"}
# mkdir -p ${SAVE_FOLDER}
# python ./multiproc.py --nproc_per_node 8 ./main.py /data/imagenet --data-backend dali-cpu --raport-file raport.json -j8 -p 100 --seed ${SEED} --lr ${INIT_LR} --optimizer-batch-size ${GLOBAL_BATCH_SIZE} --warmup ${WARMUP} --arch ${ARCH} -c fanin --label-smoothing 0.1 --lr-schedule ${LR_SCHEDULE} --mom 0.875 --wd 3.0517578125e-05 --workspace ./ -b ${LOCAL_BATCH_SIZE} ${AMP} --static-loss-scale 128 --epochs ${EPOCHS} --mixup 0.2 --widths=${WIDTH} --sp-config-file=${CONFIG_FILE} --sp-admm-sparsity-type=${SPARSITY_TYPE} ${PRUNE_ARGS} --checkpoint-dir ${SAVE_FOLDER} --log-filename=${SAVE_FOLDER}/log.txt --resume ${LOAD_CKPT}


# WIDTH=${12:-"38-77-168-312-38"}
# SAVE_FOLDER=${6:-"./checkpoints/resnet18/LTH/pretraining/ep${EPOCHS}/lr${INIT_LR}/${WIDTH}_0.59/cosine_bs_${GLOBAL_BATCH_SIZE}_${WARMUP}ep_warmup"}
# mkdir -p ${SAVE_FOLDER}
# python ./multiproc.py --nproc_per_node 8 ./main.py /data/imagenet --data-backend dali-cpu --raport-file raport.json -j8 -p 100 --seed ${SEED} --lr ${INIT_LR} --optimizer-batch-size ${GLOBAL_BATCH_SIZE} --warmup ${WARMUP} --arch ${ARCH} -c fanin --label-smoothing 0.1 --lr-schedule ${LR_SCHEDULE} --mom 0.875 --wd 3.0517578125e-05 --workspace ./ -b ${LOCAL_BATCH_SIZE} ${AMP} --static-loss-scale 128 --epochs ${EPOCHS} --mixup 0.2 --widths=${WIDTH} --sp-config-file=${CONFIG_FILE} --sp-admm-sparsity-type=${SPARSITY_TYPE} ${PRUNE_ARGS} --checkpoint-dir ${SAVE_FOLDER} --log-filename=${SAVE_FOLDER}/log.txt --resume ${LOAD_CKPT}
